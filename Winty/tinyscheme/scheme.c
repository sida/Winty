...
static pointer mk_sharp_const(scheme *sc, char *name) {
     ...
     if (!strcmp(name, "t"))
          ...
          _snprintf(tmp, sizeof(tmp), "0%s", name+1);
          sscanf(tmp, "%lo", &x);
          ...
          _snprintf(tmp, sizeof(tmp), "0x%s", name+1);
          sscanf(tmp, "%lx", &x);
          return (mk_integer(sc, x));
...
int main(int argc, char **argv) {
  ...
  //Winty固有（？）の関数追加
  scheme_define(&sc,sc.global_env,mk_symbol(&sc,"get-handle-view-wnd"),mk_foreign_func(&sc, scm_getViewhWnd));  
  scheme_define(&sc,sc.global_env,mk_symbol(&sc,"get-handle-view-dc"),mk_foreign_func(&sc, scm_getViewhDc));  
  scheme_define(&sc,sc.global_env,mk_symbol(&sc,"get-wm-command"),mk_foreign_func(&sc, scm_getWmCommand));  
